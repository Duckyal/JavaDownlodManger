    ...
    public void run() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            try {
                ...
                logManager.info("下载线程" + index, "下载完成 <" + fileSize + " >");
                break;
            } catch (Exception e) {
                if (i == 3) {
                    logManager.error("下载线程" + index, "下载失败，自动停止所有下载线程");
